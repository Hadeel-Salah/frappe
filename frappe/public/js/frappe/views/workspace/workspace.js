...
frappe.views.Workspace = class Workspace {
	...
	async setup_pages(reload) {
		...
		this.screens_links = await this.get_all_pages_links(this.sidebar_pages.pages);		
	...
	get_all_pages_links(sidebar_pages) {
		return Promise.all(sidebar_pages.map(page => this.get_pages_data(page)))
		  .then(page_data => {
			const all_links = page_data.flat().map(array => array.links).flat();
			const updated_Links = all_links.map(link => ({
			  title: link.label,
			  route: link.link_to,
			  public: 1
			}));
	...
	build_sidebar_section(title, root_pages) {
		...
		if (title == "Screens") {
			this.prepare_sidebar(this.screens_links, sidebar_section, this.sidebar);
		}else{
